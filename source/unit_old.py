...
class ImgAnalyse():
    ...
    def __init__(self,lang='ch'):
        self.ocr = PaddleOCR(use_angle_cls=True, lang=lang, show_log=False)  # need to run only once to download and load model into memory
    ...
    def getTextPosition(self,hwnd:winInfo,rangePosition,text,mode=APPROXIMATE_MATCHING,returnMode=RETURN_POSITION,isprintlog=True,message='',defaultend='\n'):
        imsrc,position=GetScrWindowsImg(hwnd,rangePosition)
        res=self.ImgAnalyse(imsrc)
        ...
        if resposition!=None:
            ...
            if mode==REPEATLY_MATCHING and returnMode==RETURN_POSITION:
                ...
                for i in resposition:
                    result.append([resposition[0][0][0]+position[0]-hwnd.x,resposition[0][0][1]+position[1]-hwnd.y])
                return result

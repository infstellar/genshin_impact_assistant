...
class CombatSettingPage(ConfigPage):
    ...
    def _autofill(self):
        ...
        for i in j:
            ...
            if cname in autofill_j:
                for k in ["position", "E_short_cd_time", "E_long_cd_time", "Elast_time", "Epress_time", "tactic_group",
                          "trigger", "Qlast_time", "Qcd_time", "vision", "long_attack_time"]:
                    if j[i][k] == "" or j[i][k] == -1:
    ...
    def _load_config_files(self):
        ...
        self.config_files.append(
            {"label": f"{CONFIGNAME_COMBAT}.json", "value": fr"config/settings/{CONFIGNAME_COMBAT}.json"})
    ...
    def _load(self):
        ...
        pin.put_select('file', self._config_file2lableAfile(self.config_files), scope="select_scope",
                       value="config/settings/Combat.json")
    ...
    def onclick_add_teamjson_withcharacters(self):
        ...
        shutil.copy(os.path.join(ROOT_PATH, "config\\tactic\\team_with_characters.uijsontemplate"),
                    os.path.join(ROOT_PATH, "config\\tactic", n + '.json'))
        self._reload_select()
        output.toast(t2t('Note: After creating a new tactic file, you must fill in all blank input boxes or `-1` input boxes'))
        pass

...
class TeyvatMove_FollowPath(FlowTemplate, TeyvatMoveCommon):
    ...
    def __init__(self, upper: TeyvatMoveFlowConnector):
        ...
        self.path_manually_modified = False
    ...
    def state_before(self):
        ...
        if 'manually_modified' not in self.additional_info.keys():
            logger.info(f'RDP optimizing')
            old_len = len(self.curr_breaks)
            from source.rdp import rdp
            self.curr_breaks = rdp(self.curr_breaks, epsilon=1)
            logger.info(f'optimized: {old_len} -> {len(self.curr_breaks)}')
    ...
    def state_motion(self):
        ...
        is_jump = False
        is_nearby = euclidean_distance(self.curr_posi, self.curr_target_pos) < 10
        ...
        self._set_inner_statement(STATEMENT_STUCK_CHECK)
    ...
    def state_adsorption(self):
        def is_ads(threshold):
            return min(euclidean_distance_plist(self.curr_posi, self.adsorptive_position)) < threshold

        # 吸附模式: 当当前距离小于允许吸附距离，开始向目标吸附点移动
        if len(self.adsorptive_position) > 0:
            adsorptive_threshold = 8
                ...
                if self.motion_state == IN_CLIMB:
                    ...
                    if 'is_cliff_collection' in self.additional_info.keys():
                        if self.additional_info['is_cliff_collection']:
                            ...
                            itt.delay(0.5)
                if self.motion_state == IN_MOVE:
                    self._exec_ads(self.curr_posi, adsorptive_threshold)
    ...
    def state_move_ahead(self):
        ...
        # delta_distance = self.CalculateTheDistanceBetweenTheAngleExtensionLineAndTheTarget(self.curr_posi,self.curr_target_pos)

        # 移动视角
        logger.trace(f"time5.1.1: change_view_to_angle:  {self.in_pt.get_diff_time()}")
        if not self.ENABLE_OPTIMIZER:
            target_degree = movement.calculate_posi2degree(self.curr_target_pos, self.curr_posi)
        else:
            target_degree = movement.calculate_posi2degree(
                self.predict_tdo_position(self.curr_posi, self.curr_target_pos), self.curr_posi)
            # target_degree = self.predict_tdo_degree(self.curr_posi, self.curr_target_pos)
    ...
    def state_in(self):
        ...
        # if self.ready_to_end:
        #     self.end_times += 1
        #     logger.debug(f"ready to end: {self.end_times} {self.offset}")
